...
function may_update() {
  ...
  # (otherwise it just does nothing) - all instances share the same server
  # installation, so updating the main instance is enough
...
function stop_server() {
  ...
  # if run processes are still alive (e.g. the signal arrived before the
  # server pidfiles existed, so stop had nothing to do), terminate them
  local RUN_PID
...
# remove state files left behind if a previous shutdown did not complete in time
rm -f "${ARK_SERVER_VOLUME}/server/ShooterGame/Saved/".*.pid \
      "${ARK_SERVER_VOLUME}/server/ShooterGame/Saved/.autorestart"

for INSTANCE_CONFIG in "${ARK_TOOLS_DIR}/instances/"*.cfg; do
